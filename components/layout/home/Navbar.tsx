...
export default function Navbar() {
  // Ambil semua items dari redux
  const items = useSelector((state: RootState) => state.cart.items);

  // Hitung jumlah qty
  const cartCount = items.reduce((sum: number, item: CartItem) => sum + item.qty, 0);
  ...
  return (
    <header className="w-full bg-white shadow-sm">
      <div className="flex items-center justify-between px-[75px] py-2">
        ...
        <Link
          href="/"
          className="flex items-center gap-1 px-6 py-2 rounded-full shadow-sm"
        >
          <span className="text-black text-[24px] font-semibold leading-6 font-sans">
            AgriculHub
          </span>
          <span className="text-black text-[16px] font-semibold leading-6 font-sans">
            ©
          </span>
        </Link>
